#!/usr/bin/env bash
# Reset Runic: kill running instances, build, package, relaunch, verify.
...
LOCK_DIR="${TMPDIR:-/tmp}/runic-compile-and-run-${LOCK_KEY}"
...
kill_claude_probes() {
  # Runic spawns `claude /usage` + `/status` in a PTY; if we kill the app mid-probe we can orphan them.
...
kill_all_runic() {
  ...
  pkill -9 -x "Runic" 2>/dev/null || true
  ...
  fail "Failed to kill all Runic instances."
}
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      log "Usage: $(basename "$0") [--wait] [--test] [--clean] [--debug-lldb] [--release-universal] [--release-arches=\"arm64 x86_64\"]"
      exit 0
      ;;
    *)
      ;;
...
if [[ "${CLEAN_BUILD}" == "1" ]]; then
  log "==> Cleaning SwiftPM state (.build + package reset)"
  rm -rf "${ROOT_DIR}/.build" || true
  # `swift package reset` clears checkouts/artifacts and is a common fix after moving/renaming a package path.
  run_step "swift package reset" swift package reset -q
fi

# 2) Kill all running Runic instances (debug, release, bundled).
log "==> Killing existing Runic instances"
kill_all_runic
kill_claude_probes
