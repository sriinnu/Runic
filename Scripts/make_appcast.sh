...
ZIP=${1:?
"Usage: $0 Runic-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/sriinnu/Runic/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Runic-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/sriinnu/Runic/releases/download/v${VERSION}/"}
