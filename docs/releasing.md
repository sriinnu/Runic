---
summary: "Runic release checklist: package, sign, notarize, appcast, and asset validation."
read_when:
  - Starting a Runic release
  - Updating signing/notarization or appcast steps
  - Validating release assets or Sparkle feed
---

# Release process (Runic)
...
- Developer ID Application cert installed (e.g. `Developer ID Application: YOUR_NAME (TEAMID)`).
...
```
./Scripts/build_icon.sh Icon.icon Runic
```
...
- Avoid `unzip` — it can add AppleDouble `._*` files that break the sealed signature and trigger “app is damaged”. Use Finder or `ditto -x -k Runic-<ver>.zip /Applications`. If Gatekeeper complains, delete the app bundle, re-extract with `ditto`, then `spctl -a -t exec` to verify.
- Manual sanity check before uploading: `find Runic.app -name '._*'` should return nothing; then `spctl --assess --type execute --verbose Runic.app` and `codesign --verify --deep --strict --verbose Runic.app` should both pass on the packaged bundle.

## Appcast (Sparkle)
After notarization:
```
SPARKLE_PRIVATE_KEY_FILE=/path/to/ed25519-priv.key \
./Scripts/make_appcast.sh Runic-0.1.0.zip \
...
The appcast feed is the committed `appcast.xml` on `main`; the GitHub release carries the app zip and dSYM zip assets.

## Tag & release
```
git tag v<version>
./Scripts/make_appcast.sh ...
# upload zip + dSYM zip to Releases
# commit and push appcast.xml to main
```

## Homebrew (Cask)
Runic ships a Homebrew **Cask** in `../homebrew-tap`. When installed via Homebrew, Runic disables Sparkle and the app
...
- [ ] `swiftformat Sources Tests --lint`, `swiftlint --strict`, `swift test` (zero warnings/errors)
...
- [ ] Upload the zip + dSYM zip to GitHub Releases; commit and push the generated `appcast.xml` to `main` so the Sparkle feed is live
- [ ] Homebrew tap: update `../homebrew-tap/Casks/runic.rb` (url + sha256), then verify:
  - `brew uninstall --cask runic || true`
  - `brew untap sriinnu/tap || true; brew tap sriinnu/tap`
  - `brew install --cask sriinnu/tap/runic && open -a Runic`
...
- [ ] Release pages: title format `Runic <version>`, notes as Markdown list (no stray blank lines)
- [ ] Changelog/release notes are user-facing: avoid internal-only bullets (build numbers, script bumps) and keep entries concise
- [ ] Download uploaded `Runic-<ver>.zip`, unzip via `ditto`, run, and verify signature (`spctl -a -t exec -vv Runic.app` + `stapler validate`)
- [ ] Confirm `appcast.xml` points to the new zip/version and renders the HTML release notes (not escaped tags)
- [ ] Verify on GitHub Releases: assets present (zip, dSYM zip), release notes match changelog, version/tag correct
...
- [ ] Keep a previous signed build in `/Applications/Runic.app` to test Sparkle delta/full update to the new release
- [ ] Manual Gatekeeper sanity: after packaging, `find Runic.app -name '._*'` is empty, `spctl --assess --type execute --verbose Runic.app` and `codesign --verify --deep --strict --verbose Runic.app` succeed
- [ ] For Sparkle verification: if replacing `/Applications/Runic.app`, quit first, replace, relaunch, and test update
