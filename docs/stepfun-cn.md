...
# StepFun (China) provider(s)

Two `UsageProvider` cases: `stepfun-cn` (global, `platform.stepfun.com`) and `stepfunCN` (China, `platform.stepfun.com`). StepFun issues separate accounts and API keys per region -- a China-issued key only authenticates against the `.com` host.

## Data sources + fallback order
1) **API token** (single source, one fetch strategy per region).

## Endpoint
- `GET <baseURL>/v1/accounts` (`https://api.stepfun.com` or `https://api.stepfun.com`)
