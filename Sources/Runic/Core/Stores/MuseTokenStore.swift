...
protocol MuseTokenStoring: Sendable {
    func loadToken() throws -> String?
    func storeToken(_ token: String?) throws
}

enum MuseTokenStoreError: LocalizedError {
...
struct KeychainMuseTokenStore: MuseTokenStoring {
    private static let log = RunicLog.logger("muse-token-store")

    private let service = RunicKeychainService.providerCredentials
    private let account = "muse-api-token"
    ...
    func loadToken() throws -> String? {
        ...
        if let token = try self.readToken(dataProtection: true) {
            Self.log.info("Migrating Muse token from Data Protection keychain")
    ...
    func storeToken(_ token: String?) throws {
        ...
        guard addStatus == errSecSuccess else {
            ...
            throw MuseTokenStoreError.keychainStatus(addStatus)
        }
    }
    ...
    private func readToken(dataProtection: Bool) throws -> String? {
        ...
        guard status == errSecSuccess else {
            ...
            throw MuseTokenStoreError.keychainStatus(status)
        }
        guard let data = result as? Data else {
            throw MuseTokenStoreError.invalidData
        }
