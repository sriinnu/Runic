...
protocol ClineTokenStoring: Sendable {
    func loadToken() throws -> String?
    func storeToken(_ token: String?) throws
}

enum ClineTokenStoreError: LocalizedError {
...
struct KeychainClineTokenStore: ClineTokenStoring {
    private static let log = RunicLog.logger("cline-token-store")

    private let service = RunicKeychainService.providerCredentials
    private let account = "cline-api-token"
    ...
    func loadToken() throws -> String? {
        ...
        if let token = try self.readToken(dataProtection: true) {
            Self.log.info("Migrating Cline token from Data Protection keychain")
    ...
    func storeToken(_ token: String?) throws {
        ...
        guard addStatus == errSecSuccess else {
            ...
            throw ClineTokenStoreError.keychainStatus(addStatus)
        }
    }
    ...
    private func readToken(dataProtection: Bool) throws -> String? {
        ...
        guard status == errSecSuccess else {
            ...
            throw ClineTokenStoreError.keychainStatus(status)
        }
        guard let data = result as? Data else {
            throw ClineTokenStoreError.invalidData
        }
