...
protocol AuggieTokenStoring: Sendable {
    func loadToken() throws -> String?
    func storeToken(_ token: String?) throws
}

enum AuggieTokenStoreError: LocalizedError {
...
struct KeychainAuggieTokenStore: AuggieTokenStoring {
    private static let log = RunicLog.logger("auggie-token-store")

    private let service = "com.sriinnu.athena.Runic"
    private let account = "auggie-api-token"
    ...
    func loadToken() throws -> String? {
        ...
        if let token = try self.readToken(dataProtection: true) {
            Self.log.info("Migrating Auggie token from Data Protection keychain")
    ...
    func storeToken(_ token: String?) throws {
        ...
        guard addStatus == errSecSuccess else {
            ...
            throw AuggieTokenStoreError.keychainStatus(addStatus)
        }
    }
    ...
    private func readToken(dataProtection: Bool) throws -> String? {
        ...
        guard status == errSecSuccess else {
            ...
            throw AuggieTokenStoreError.keychainStatus(status)
        }
        guard let data = result as? Data else {
            throw AuggieTokenStoreError.invalidData
        }
