...
enum QwenTokenStoreError: LocalizedError {
...
struct KeychainQwenTokenStore: QwenTokenStoring {
    private static let log = RunicLog.logger("qwen-token-store")

    private let service = "com.sriinnu.athena.Runic"
    private let account = "qwen-api-token"
    ...
    func loadToken() throws -> String? {
        ...
        if let token = try self.readToken(dataProtection: true) {
            Self.log.info("Migrating Qwen token from Data Protection keychain")
    ...
    func storeToken(_ token: String?) throws {
        ...
        guard addStatus == errSecSuccess else {
            ...
            throw QwenTokenStoreError.keychainStatus(addStatus)
        }
    }
    ...
    private func readToken(dataProtection: Bool) throws -> String? {
        ...
        guard status == errSecSuccess else {
            ...
            throw QwenTokenStoreError.keychainStatus(status)
        }
        guard let data = result as? Data else {
            throw QwenTokenStoreError.invalidData
        }
