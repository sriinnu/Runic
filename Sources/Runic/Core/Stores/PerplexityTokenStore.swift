...
protocol PerplexityTokenStoring: Sendable {
    func loadToken() throws -> String?
    func storeToken(_ token: String?) throws
}

enum PerplexityTokenStoreError: LocalizedError {
...
struct KeychainPerplexityTokenStore: PerplexityTokenStoring {
    ...
    func loadToken() throws -> String? {
        ...
        if let token = try self.readToken(dataProtection: true) {
            Self.log.info("Migrating Perplexity token from Data Protection keychain")
    ...
    func storeToken(_ token: String?) throws {
        ...
        guard addStatus == errSecSuccess else {
            ...
            throw PerplexityTokenStoreError.keychainStatus(addStatus)
        }
    }
    ...
    private func readToken(dataProtection: Bool) throws -> String? {
        ...
        guard status == errSecSuccess else {
            ...
            throw PerplexityTokenStoreError.keychainStatus(status)
        }
        guard let data = result as? Data else {
            throw PerplexityTokenStoreError.invalidData
        }
