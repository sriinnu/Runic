...
protocol SambaNovaTokenStoring: Sendable {
    func loadToken() throws -> String?
    func storeToken(_ token: String?) throws
}

enum SambaNovaTokenStoreError: LocalizedError {
...
struct KeychainSambaNovaTokenStore: SambaNovaTokenStoring {
    ...
    func loadToken() throws -> String? {
        ...
        if let token = try self.readToken(dataProtection: true) {
            Self.log.info("Migrating SambaNova token from Data Protection keychain")
    ...
    private func readToken(dataProtection: Bool) throws -> String? {
        ...
        guard let data = result as? Data else {
            throw SambaNovaTokenStoreError.invalidData
        }
