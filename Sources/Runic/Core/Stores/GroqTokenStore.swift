...
protocol GroqTokenStoring: Sendable {
    func loadToken() throws -> String?
    func storeToken(_ token: String?) throws
}

enum GroqTokenStoreError: LocalizedError {
...
struct KeychainGroqTokenStore: GroqTokenStoring {
    ...
    func loadToken() throws -> String? {
        ...
        if let token = try self.readToken(dataProtection: true) {
            Self.log.info("Migrating Groq token from Data Protection keychain")
    ...
    func storeToken(_ token: String?) throws {
        ...
        guard addStatus == errSecSuccess else {
            ...
            throw GroqTokenStoreError.keychainStatus(addStatus)
        }
    }
    ...
    private func readToken(dataProtection: Bool) throws -> String? {
        ...
        guard status == errSecSuccess else {
            ...
            throw GroqTokenStoreError.keychainStatus(status)
        }
        guard let data = result as? Data else {
            throw GroqTokenStoreError.invalidData
        }
