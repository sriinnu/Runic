...
protocol AzureOpenAITokenStoring: Sendable {
    func loadToken() throws -> String?
    func storeToken(_ token: String?) throws
}

enum AzureOpenAITokenStoreError: LocalizedError {
...
struct KeychainAzureOpenAITokenStore: AzureOpenAITokenStoring {
    ...
    func loadToken() throws -> String? {
        ...
        if let token = try self.readToken(dataProtection: true) {
            Self.log.info("Migrating Azure OpenAI token from Data Protection keychain")
    ...
    func storeToken(_ token: String?) throws {
        ...
        guard addStatus == errSecSuccess else {
            ...
            throw AzureOpenAITokenStoreError.keychainStatus(addStatus)
        }
    }
    ...
    private func readToken(dataProtection: Bool) throws -> String? {
        ...
        guard status == errSecSuccess else {
            ...
            throw AzureOpenAITokenStoreError.keychainStatus(status)
        }
        guard let data = result as? Data else {
            throw AzureOpenAITokenStoreError.invalidData
        }
