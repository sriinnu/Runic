...
protocol MistralTokenStoring: Sendable {
    func loadToken() throws -> String?
    func storeToken(_ token: String?) throws
}

enum MistralTokenStoreError: LocalizedError {
...
struct KeychainMistralTokenStore: MistralTokenStoring {
    private static let log = RunicLog.logger("mistral-token-store")

    private let service = "com.sriinnu.athena.Runic"
    private let account = "mistral-api-token"
    ...
    func loadToken() throws -> String? {
        ...
        if let token = try self.readToken(dataProtection: true) {
            Self.log.info("Migrating Mistral token from Data Protection keychain")
    ...
    func storeToken(_ token: String?) throws {
        ...
        guard addStatus == errSecSuccess else {
            ...
            throw MistralTokenStoreError.keychainStatus(addStatus)
        }
    }
    ...
    private func readToken(dataProtection: Bool) throws -> String? {
        ...
        guard status == errSecSuccess else {
            ...
            throw MistralTokenStoreError.keychainStatus(status)
        }
        guard let data = result as? Data else {
            throw MistralTokenStoreError.invalidData
        }
