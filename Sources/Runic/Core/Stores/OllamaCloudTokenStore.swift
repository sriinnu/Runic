...
protocol OllamaCloudTokenStoring: Sendable {
    func loadToken() throws -> String?
    func storeToken(_ token: String?) throws
}

enum OllamaCloudTokenStoreError: LocalizedError {
...
struct KeychainOllamaCloudTokenStore: OllamaCloudTokenStoring {
    private static let log = RunicLog.logger("ollama-cloud-token-store")

    private let service = RunicKeychainService.providerCredentials
    private let account = "ollama-cloud-api-token"
    ...
    func loadToken() throws -> String? {
        ...
        if let token = try self.readToken(dataProtection: true) {
            Self.log.info("Migrating Ollama Cloud token from Data Protection keychain")
    ...
    func storeToken(_ token: String?) throws {
        ...
        guard addStatus == errSecSuccess else {
            ...
            throw OllamaCloudTokenStoreError.keychainStatus(addStatus)
        }
    }
    ...
    private func readToken(dataProtection: Bool) throws -> String? {
        ...
        guard status == errSecSuccess else {
            ...
            throw OllamaCloudTokenStoreError.keychainStatus(status)
        }
        guard let data = result as? Data else {
            throw OllamaCloudTokenStoreError.invalidData
        }
