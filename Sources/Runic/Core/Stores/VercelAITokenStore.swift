...
protocol VercelAITokenStoring: Sendable {
    func loadToken() throws -> String?
    func storeToken(_ token: String?) throws
}

enum VercelAITokenStoreError: LocalizedError {
...
struct KeychainVercelAITokenStore: VercelAITokenStoring {
    ...
    func loadToken() throws -> String? {
        ...
        if let token = try self.readToken(dataProtection: true) {
            Self.log.info("Migrating Vercel AI token from Data Protection keychain")
    ...
    private func readToken(dataProtection: Bool) throws -> String? {
        ...
        guard let data = result as? Data else {
            throw VercelAITokenStoreError.invalidData
        }
