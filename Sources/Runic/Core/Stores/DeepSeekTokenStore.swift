...
protocol DeepSeekTokenStoring: Sendable {
    func loadToken() throws -> String?
    func storeToken(_ token: String?) throws
}

enum DeepSeekTokenStoreError: LocalizedError {
...
struct KeychainDeepSeekTokenStore: DeepSeekTokenStoring {
    ...
    func loadToken() throws -> String? {
        ...
        if let token = try self.readToken(dataProtection: true) {
            Self.log.info("Migrating DeepSeek token from Data Protection keychain")
    ...
    func storeToken(_ token: String?) throws {
        ...
        guard addStatus == errSecSuccess else {
            ...
            throw DeepSeekTokenStoreError.keychainStatus(addStatus)
        }
    }
    ...
    private func readToken(dataProtection: Bool) throws -> String? {
        ...
        guard status == errSecSuccess else {
            ...
            throw DeepSeekTokenStoreError.keychainStatus(status)
        }
        guard let data = result as? Data else {
            throw DeepSeekTokenStoreError.invalidData
        }
