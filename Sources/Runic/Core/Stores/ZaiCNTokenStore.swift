...
protocol ZaiCNTokenStoring: Sendable {
    func loadToken() throws -> String?
    func storeToken(_ token: String?) throws
}

struct KeychainZaiCNTokenStore: ZaiCNTokenStoring {
    private static let log = RunicLog.logger("zai-cn-token-store")

    private let service = RunicKeychainService.providerCredentials
    private let account = "zai-cn-api-token"
