...
protocol FireworksTokenStoring: Sendable {
    func loadToken() throws -> String?
    func storeToken(_ token: String?) throws
}

enum FireworksTokenStoreError: LocalizedError {
...
struct KeychainFireworksTokenStore: FireworksTokenStoring {
    private static let log = RunicLog.logger("fireworks-token-store")

    private let service = "com.sriinnu.athena.Runic"
    private let account = "fireworks-api-token"
    ...
    func loadToken() throws -> String? {
        ...
        if let token = try self.readToken(dataProtection: true) {
            Self.log.info("Migrating Fireworks token from Data Protection keychain")
    ...
    func storeToken(_ token: String?) throws {
        ...
        guard addStatus == errSecSuccess else {
            ...
            throw FireworksTokenStoreError.keychainStatus(addStatus)
        }
    }
    ...
    private func readToken(dataProtection: Bool) throws -> String? {
        ...
        guard status == errSecSuccess else {
            ...
            throw FireworksTokenStoreError.keychainStatus(status)
        }
        guard let data = result as? Data else {
            throw FireworksTokenStoreError.invalidData
        }
