...
protocol CohereTokenStoring: Sendable {
    func loadToken() throws -> String?
    func storeToken(_ token: String?) throws
}

enum CohereTokenStoreError: LocalizedError {
...
struct KeychainCohereTokenStore: CohereTokenStoring {
    private static let log = RunicLog.logger("cohere-token-store")

    private let service = "com.sriinnu.athena.Runic"
    private let account = "cohere-api-token"
    ...
    func loadToken() throws -> String? {
        ...
        if let token = try self.readToken(dataProtection: true) {
            Self.log.info("Migrating Cohere token from Data Protection keychain")
    ...
    func storeToken(_ token: String?) throws {
        ...
        guard addStatus == errSecSuccess else {
            ...
            throw CohereTokenStoreError.keychainStatus(addStatus)
        }
    }
    ...
    private func readToken(dataProtection: Bool) throws -> String? {
        ...
        guard status == errSecSuccess else {
            ...
            throw CohereTokenStoreError.keychainStatus(status)
        }
        guard let data = result as? Data else {
            throw CohereTokenStoreError.invalidData
        }
