...
// - Single ping per menu open (not continuous polling)
// - Cache all results aggressively
// - Stop animations immediately when data loaded or error reached
//
// **Performance Targets**:
// - Menubar icon: 60 FPS (smooth on modern Macs)
// - Popover menu: 60 FPS (full detail)
// - Animation stops: when data loaded OR error state reached
// - No always-running timers
//
// **Resource Management**:
// - DisplayLink stops automatically when done
// - Minimal CPU/GPU usage
// - Smart caching prevents redundant renders

extension StatusItemController {
    func needsMenuBarIconAnimation() -> Bool {
        // Performance: Only animate when actively fetching, stop when data loaded
    ...
    func updateBlinkingState() {
        ...
        // Performance: Cache provider state to minimize repeated lookups (60fps smooth)
    ...
    private func tickBlink(now: Date = .init()) {
        ...
        let blinkDuration: TimeInterval = PerformanceConstants.blinkDuration
        let doubleBlinkChance = PerformanceConstants.doubleBlinkChance
    ...
    func blinkAmount(for provider: UsageProvider) -> CGFloat {
        guard self.isBlinkingAllowed() else { return 0 }
        return self.blinkAmounts[provider] ?? 0
    }

    func wiggleAmount(for provider: UsageProvider) -> CGFloat {
        guard self.isBlinkingAllowed() else { return 0 }
        return self.wiggleAmounts[provider] ?? 0
    }

    func tiltAmount(for provider: UsageProvider) -> CGFloat {
        guard self.isBlinkingAllowed() else { return 0 }
    ...
    /// **Performance-Critical**: Determines if animation should run for a provider
    /// - Only animates during active data fetching (no always-on loops)
    /// - Stops immediately when data loaded OR error state reached
    /// - Prevents battery drain from unnecessary rendering
    /// - Zero token leakage: uses cached data first, minimal API pings
    ...
    /// **Performance Optimization**: Manages animation lifecycle with strict resource control
    /// - Menubar: 30 FPS cap (battery-efficient)
    /// - Auto-stops when data loaded or error reached
    /// - No always-running timers
    /// - Minimal CPU/GPU usage
    func updateAnimationState() {
        ...
        if needsAnimation {
            if self.animationDriver == nil {
                ...
                // Performance: 60 FPS for smooth menubar animations
                driver.start(fps: PerformanceConstants.menubarFPS)
            } else if let forced = self.settings.debugLoadingPattern, forced != self.animationPattern {
                self.animationPattern = forced
                self.animationPhase = 0
            }
        } else {
            // Critical: Stop animation immediately when data loaded/error reached
