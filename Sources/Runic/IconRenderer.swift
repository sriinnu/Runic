        ...
        dataMode: IconDataMode = .remaining) -> NSImage
    {
        ...
        let render = {
            self.renderImage(isTemplate: appearance == .template) {
                ...
                let baseFill = appearance == .template ? NSColor.labelColor : accentColor
                ...
                func drawBar(
                    ...
                    remaining: Double?,
                    alpha: CGFloat = 1.0,
                    sigil: SigilStyle? = nil)
                {
                    let chamferPx = max(1, min(3, rectPx.h / 3))
                    let trackPath = Self.chamferedPath(rectPx: rectPx, chamferPx: chamferPx)
                    ...
                    let strokeRectPx = RectPx(
                        x: rectPx.x + insetPx,
                        y: rectPx.y + insetPx,
                        w: max(0, rectPx.w - insetPx * 2),
                        h: max(0, rectPx.h - insetPx * 2))
                    let strokeChamferPx = max(0, chamferPx - insetPx)
                    let strokePath = Self.chamferedPath(rectPx: strokeRectPx, chamferPx: strokeChamferPx)
                    strokePath.lineWidth = CGFloat(strokeWidthPx) / Self.outputScale
                    baseFill.withAlphaComponent(trackStrokeAlpha * alpha).setStroke()
                    strokePath.stroke()

                    // Fill: clip to the track and paint a left-to-right rect so the progress edge is straight.
                    if let remaining {
                    ...
                    if let sigil {
                        Self.drawSigil(sigil, in: rectPx)
                    }
                }
                ...
                let sigil = Self.sigilStyle(for: style)
