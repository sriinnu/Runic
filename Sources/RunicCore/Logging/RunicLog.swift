...
public enum RunicLog {
    ...
    public static func logger(_ category: String) -> RunicLogger {
        let logger = Logger(label: "com.sriinnu.athena.runic.\(category)")
        return RunicLogger { level, message, metadata in
...
public struct RunicLogger: Sendable {
    private let logFn: @Sendable (RunicLog.Level, String, [String: String]?) -> Void

    fileprivate init(_ logFn: @escaping @Sendable (RunicLog.Level, String, [String: String]?) -> Void) {
        self.logFn = logFn
    }
