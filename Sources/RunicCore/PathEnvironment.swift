...
public enum BinaryLocator {
        ...
        home: String) -> String?
    {
        ...
        // 5) Minimal fallback
        // GUI-launched apps often miss user-installed tooling paths (Homebrew/npm/bun/mise/asdf/volta).
        // Keep this conservative but practical.
        let fallback = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "\(home)/.local/bin",
...
public enum ShellCommandLocator {
    ...
    private static func runShellCapture(_ shell: String?, _ timeout: TimeInterval, _ command: String) -> String? {
        ...
        // Never let stderr backpressure block shell init (some configs print warnings).
...
enum LoginShellPathCapturer {
        ...
        timeout: TimeInterval = 2.0) -> [String]?
    {
        ...
        let stdout = Pipe()
        process.standardOutput = stdout
        // Avoid deadlocks if shell init writes to stderr.
        process.standardError = FileHandle.nullDevice
        do {
            try process.run()
        } catch {
