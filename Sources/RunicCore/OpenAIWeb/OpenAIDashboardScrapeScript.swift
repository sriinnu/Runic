...
let openAIDashboardScrapeScript = """
    ...
    (() => {
      ...
      const usageBreakdownJSON = (() => {
        try {
          if (window.__runicUsageBreakdownJSON) return window.__runicUsageBreakdownJSON;
          ...
          window.__runicUsageBreakdownJSON = json;
          window.__runicUsageBreakdownDebug = json ? null : JSON.stringify(debug);
          return json;
        } catch {
          return null;
        }
      ...
      const usageBreakdownDebug = (() => {
        try {
          return window.__runicUsageBreakdownDebug || null;
        } catch {
          return null;
        }
      })();
      ...
      try {
        ...
        if (header) {
          ...
          if (rows.length === 0 && !window.__runicDidScrollToCredits) {
            window.__runicDidScrollToCredits = true;
        ...
        } else if (rows.length === 0 && !window.__runicDidScrollToCredits && scrollHeight > viewportHeight * 1.5) {
