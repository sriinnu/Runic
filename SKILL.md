...
Runic prefers Kosha-discovery 1.6.2's local TTL-backed registry at `~/.kosha/registry.json` for model/provider context capacity metadata. If that registry is missing or stale, Runic falls back to `Sources/Runic/Resources/provider-context-windows.json`.
